...
바로 이 사수가 변수에 사용될 떄, 즉 복사가 되는 시점에 타입이 정해진다.
...
원래대로라면 -1이 맞겠지만 놀랍게도 255가 찍힌다. 이것을 `Underflow`라고 한다.      
...
놀랍게도 `100 - 200 > 0`에서 `Underflow`가 발생해버리기 때문에 `-100 > 0`가 true로 떨어지게 된다. 
