...
여기서 쇼트 서킷은 양변 중 우변이 `false`라면 이 조건은 최종적으로 결과가 `false`로 떨어지게 된다.

이것은 좌변이 `true`인지 `false`인지 검사할 필요도 없다. 
...
여기서 쇼트 서킷은 양변 중 우변이 `true`라면 좌변이 `true`인지 `false`인지 검사하지 않는다.
...
심플하지만 사실 중괄호를 사용하지 않고 자바와는 달리 다양한 형식을 제공하기 때문한다.    
